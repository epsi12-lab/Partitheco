        ...
        if (results.length > 0) {
            container.innerHTML = results.map(item => `
                ...
                ">
                    ...
                    ${item.author ? `<br><small style="color: #666;">${escapeHtml(item.author)}</small>` : ''}
                </a>
            `).join('');
...
function highlightMatch(text, query) {
    ...
    return escapedText.replace(regex, '<mark style="background: var(--color-lit-gold, #ffeb3b); padding: 0 2px;">$1</mark>');
}
