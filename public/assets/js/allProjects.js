...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function createProjectCard(p, idx) {
    ...
    if (['jpg','jpeg','png','gif'].includes(ext)) {
      html += `<a href="javascript:void(0)" onclick="openLightbox('${safeFileUrl}')">
                 <img src="${safeFileUrl}" class="project-thumbnail">
               </a>`;
    } else if (ext === 'pdf') {
      html += `<a href="javascript:void(0)" onclick="openPDF('${safeFileUrl}')">
                 <img src="assets/static/file-pdf-solid.svg" class="project-thumbnail pdf-icon">
               </a>`;
    }
    html += `<h3><a href="${detail}">${escapeHtml(p.title)}</a></h3>`;
