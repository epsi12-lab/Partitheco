...
$media       = $project['media'];
$moment      = $project['moment_messe'] ?? '';
$temps       = $project['temps_liturgique'] ?? '';
$voix        = $project['voix'] ?? '';
$is_lit      = $project['is_liturgical'] ?? 0;
  ...
  <section class="animated-form create-section">
    ...
    <form action="edit.php?id=<?= $projectId ?>&lang=<?= htmlspecialchars($lang) ?>"
      ...
      <div class="form-group" style="--delay:<?= $delay ?>s">
        ...
        <div class="form-line"></div><p class="error-message"></p>
        <?php if ($thumbnail): ?>
          <p class="current-file">Fichier actuel : <?= htmlspecialchars($thumbnail) ?></p>
        <?php endif; ?>
      </div>
      ...
      <div class="form-group" style="--delay:<?= $delay ?>s">
        ...
        <div class="form-line"></div><p class="error-message"></p>
        <?php if ($media): ?>
          <p class="current-file">Média actuel : <?= htmlspecialchars($media) ?></p>
        <?php endif; ?>
      </div>
